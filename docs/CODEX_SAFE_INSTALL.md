...
- `CHE_ICAL_MCP_CONFIRMATION_TOKEN` requires write-safe and destructive tool calls to include a matching `confirmation_token` argument. Use a per-session token and keep Codex tool approval enabled. If this variable is present but empty or whitespace-only, the server fails closed.
